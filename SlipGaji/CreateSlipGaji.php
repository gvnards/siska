...
class CreateSlipGaji extends Conn {
  public function InsertSlipGaji () {
    ...
    // insert SLIP GAJI
    $sth = $dbh->prepare("INSERT INTO slip_gaji(nip, nama, jenis, golongan, tunjangan_jabatan, tunjangan_suami_istri, tunjangan_anak, id_potongan_lainlain, tanggal_slip, total_gaji, total_tunjangan, total_potongan) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $sth->execute([$nip, $nama, $jenis, $golongan, $tunjangan_jabatan, $tunjangan_suami_istri, $tunjangan_anak, $id_potongan_lainlain, $tanggal_slip, $total_gaji, $total_tunjangan, $total_potongan]);
